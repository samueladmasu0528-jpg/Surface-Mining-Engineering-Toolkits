...
function calculateSlopeFoS(state, isSaturated) {
  ...
  // Setup typical fail circle intersection geometries passing through slope profile
  const tanBeta = Math.tan(beta);
  const toeX = h / tanBeta + 50;
  const toeY = 50; 
  const crestX = 50;

  const centerX = toeX - 0.2 * h;
  const centerY = toeY + 1.4 * h;
  const radius = Math.sqrt(Math.pow(centerX - toeX, 2) + Math.pow(centerY - toeY, 2));

  // Partition structural body into vertical slices to integrate forces
  const numSlices = 10;
  ...
  for (let k = 0; k < numSlices; k++) {
    ...
    const alphaRad = Math.asin(Math.max(-0.99, Math.min(0.99, (sliceMidX - centerX) / radius)));
    const dl = sliceWidth / Math.cos(alphaRad);

    let u = 0;
    if (isSaturated) {
      const phreaticY = (toeY + h) - waterDepth;
      if (slipY < phreaticY) {
        // Pore water pressure reduction acts as uplift: u = water column * unit weight of water (approx. 9.81 kN/m³)
        u = Math.max(0, (phreaticY - slipY) * 9.81);
      }
    }

    const N_eff = Math.max(0, weight * Math.cos(alphaRad) - u * dl);
    const resisting = cohesion * dl + N_eff * Math.tan(phi);
    const driving = weight * Math.sin(alphaRad);

    sumResisting += resisting;
    sumDriving += driving;
  }

  if (sumDriving <= 0.1) return 9.99;
  const calculated = sumResisting / sumDriving;
  return isNaN(calculated) || calculated <= 0 ? 0.01 : Math.min(9.99, calculated);
}

// Pure, reusable ore-blend optimizer so every page that needs a blend result
// (the blending tool itself and the report generator) computes it the same way
// instead of one of them hard-coding a placeholder value.
function computeOptimalBlend(sources, targetAsh, targetMoisture, targetCV) {
  if (!sources || sources.length === 0) {
    return { ratios: [], qualities: null };
  }
  ...
  const evaluateError = (ratios) => {
    ...
    ratios.forEach((ratio, index) => {
      mixedAsh += ratio * (parseFloat(sources[index]?.ash) || 0);
      mixedMoist += ratio * (parseFloat(sources[index]?.moisture) || 0);
      mixedCV += ratio * (parseFloat(sources[index]?.cv) || 0);
    });
    return (
      wAsh * Math.pow(mixedAsh - targetAsh, 2) +
      wMoisture * Math.pow(mixedMoist - targetMoisture, 2) +
      wCV * Math.pow(mixedCV - targetCV, 2)
    );
  };
  ...
  bestX.forEach((ratio, index) => {
    finalAsh += ratio * (parseFloat(sources[index]?.ash) || 0);
    finalMoist += ratio * (parseFloat(sources[index]?.moisture) || 0);
    finalCV += ratio * (parseFloat(sources[index]?.cv) || 0);
  });

  return { ratios: bestX, qualities: { ash: finalAsh, moisture: finalMoist, cv: finalCV } };
}
...
export default function App() {
  ...
  // NOTE: browser storage (localStorage/sessionStorage) is not available in this
  // environment, so session data lives purely in React state for the lifetime
  // of the app instead of persisting across reloads.
...
function SlopeStabilityPage({ state, setState }) {
  ...
  useEffect(() => {
    ...
    const pxCenterX = pxToeX - 0.2 * height * scale;
    ...
    ctx.arc(pxCenterX, pxCenterY, Math.max(1, pxRadius), Math.PI + 0.1, endAngle);
...
function GradeBlendingPage({ state, setState }) {
  ...
  useEffect(() => {
    const { ratios, qualities } = computeOptimalBlend(state.sources, targetAsh, targetMoisture, targetCV);
    setBlendRatios(ratios);
    setResultingQualities(qualities);
    setIsInfeasible(qualities ? (Math.abs(qualities.ash - targetAsh) > 2.0 || Math.abs(qualities.cv - targetCV) > 250) : false);
...
function ReportGeneratorPage({ userProfile, strippingRatio, fleet, capexOpex, slope, blending }) {
  ...
  // Compute the actual optimized blend from the live Grade Blending state
  // instead of reporting a hard-coded placeholder ash value.
  const blendTargetAsh = parseFloat(blending.targetAsh) || 0;
  const blendTargetMoisture = parseFloat(blending.targetMoisture) || 0;
  const blendTargetCV = parseFloat(blending.targetCV) || 0;
  const blendResult = useMemo(
    () => computeOptimalBlend(blending.sources, blendTargetAsh, blendTargetMoisture, blendTargetCV),
    [blending.sources, blendTargetAsh, blendTargetMoisture, blendTargetCV]
  );
  const finalAshValue = blendResult.qualities ? blendResult.qualities.ash.toFixed(1) : '0.0';
